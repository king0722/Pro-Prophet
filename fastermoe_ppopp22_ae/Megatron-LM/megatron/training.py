...
def forward_and_backward_steps_with_communication(forward_step_func, data_iterator, model,
    ...
    print('{}: 1f1b start'.format(torch.distributed.get_rank()))
    # Forward model for one step.
    timers('forward-compute').start()
    output_tensor = forward_step_func(data_iterator, model, input_tensor)
    timers('forward-compute').stop()
    print('{}: 1f1b forward'.format(torch.distributed.get_rank()))
    ...
    print('{}: 1f1b forward send backward recv'.format(torch.distributed.get_rank()))
    ...
    input_grad_tensor = \
        backward_step(optimizer, model, input_tensor, output_tensor, output_tensor_grad)
    timers('backward-compute').stop()
    print('{}: 1f1b backward step'.format(torch.distributed.get_rank()))
